...
```bash Bash
curl https://metlo-releases.s3.us-west-2.amazonaws.com/metlo_agent_linux_amd64_alpine_0.1.1 > /usr/local/bin/metlo-agent
apk update && apk add libgcc
```

**Ubuntu**

```bash Bash
curl https://metlo-releases.s3.us-west-2.amazonaws.com/metlo_agent_linux_amd64_ubuntu_jammy.1.1 > /usr/local/bin/metlo-agent
```

**Debian**

```bash Bash
curl https://metlo-releases.s3.us-west-2.amazonaws.com/metlo_agent_linux_amd64_debian_bullseye_0.1.1 > /usr/local/bin/metlo-agent
```
